...
//		basicSearch();
	...
	private static void customClassSearch() {
		...
		int index = Arrays.binarySearch(members, 
				new Member("홍길동"),		//	비교할 객체 생성. members 배열 안의 홍길동과는 다른 객체임 
	...
	//	기본 자료형의 오름차순 정렬
	private static void basicSortAsc() {	//	ascending (오름차순 0 -> 9)
	...
	//	기본 자료형의 내림차순 정렬
	private static void basicSortDesc() {	//	descending (내림차순 9 -> 0)
		Integer[] scores = { 80, 50, 30, 90, 75, 88, 77 };		//	Integer - 객체화
		System.out.println("원본 : " + Arrays.toString(scores));
		//	역순 정렬
		Arrays.sort(scores, Collections.reverseOrder());		//	reverseOrder - 기본타입 지원하지 않음
	...
	private static void arrayCopyEx() {
		...
		System.out.println("System 을 이용한 배열 복사 : " + Arrays.toString(target));
		
		//	Arrays 클래스를 이용한 배열의 복사
		target = Arrays.copyOf(src, src.length);	//	src 배열을 길이만큼 복제
		System.out.println("copyOf : " + Arrays.toString(target));
		
		//	배열의 일부 복제
		target = Arrays.copyOfRange(src,	//	월본 배열 
									5, 		//	복사 시작 인덱스
									12);	//	복사 끝 인덱스
